...
pip install torch==2.5.1 torchvision==0.20.1 torchaudio==2.5.1 --index-url https://download.pytorch.org/whl/cu124 # you might need to change the cuda version here
...
## Part I: Camera Extrinsics Calibration (30%)

The first step is to calibrate the extrinsics of the camera. Since we mount the camera on the gripper, we need to perform eye-in-hand calibration to get the extrinsics.

We provide a chessboard in the simulation environment, and you need to move the robot to different poses, capture images of the chessboard, compute the target-to-camera transformation, and finally estimate the camera pose in the gripper end-effector frame.

Please carefully read the comments in ```camera_calib.py``` and complete the ```main()```. You will get 30% if you can get translation error < 0.01 m and rotation error < 2.0 deg.

## Part II: Directly Estimate the Object Pose (25%)
...
You will get 25% if the mean translation error is smaller than 0.025 m and the mean rotation error is smaller than 0.25 rad.

## Part III: Estimate the Object Pose by Fitting (25%)
...
In this part, you will get 25% if the mean translation error is smaller than 0.01 m and the mean rotation error is smaller than 0.25 rad.

## Part IV: Grasp the Object in the Simulation (20%)
...
You will get 10% for each part above if you get a success rate >= 80%.
...
If you install some packages that are not in the ```Environment``` part, please add an ```Install.md``` to show how to install them.
