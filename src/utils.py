...
def transform_grasp_pose(
    ...
    """
    obj_pose_cam = to_pose(est_trans, est_rot)
    cam_pose_robot = to_pose(cam_trans, cam_rot)
    grasp_pose_obj = to_pose(grasp.trans, grasp.rot)
    grasp_pose_robot = cam_pose_robot @ obj_pose_cam @ grasp_pose_obj
    return Grasp(
        trans=grasp_pose_robot[:3, 3],
        rot=grasp_pose_robot[:3, :3],
        width=grasp.width,
    )
